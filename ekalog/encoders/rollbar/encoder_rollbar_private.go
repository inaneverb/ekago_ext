...
// doBuild builds the current CI_RollbarEncoder only if it has not built yet.
// There is no-op if encoder already built.
func (re *CI_RollbarEncoder) doBuild(env, codeVer string) *CI_RollbarEncoder {
...
// encode is an entry point of encoding log's entry to the JSON for Rollbar service.
// It's the function that is returned by FreezeAndGetEncoder() method.
func (re *CI_RollbarEncoder) encode(e *ekalog.Entry) []byte {

	s := re.api.BorrowStream(nil)
	defer re.api.ReturnStream(s)
...
//   - e's level, using:
//       - "debug" for ekalog.LEVEL_DEBUG,
//       - "info" for ekalog.LEVEL_INFO,
//       - "warning" for ekalog.LEVEL_WARNING,
//       - "error" for ekalog.LEVEL_ERROR, or for any custom log levels
//       - "critical" for ekalog.LEVEL_FATAL
//   - e's timestamp (as unix in sec),
//   - Rollbar's env,
//   - Rollbar's code version (if presented),
//   -
//
// Puts JSON encoded data into 's' stream, skipping empty log's message
// and error's public message if 'allowEmpty' is false.
func (re *CI_RollbarEncoder) encodeBase(
	...
	levelEncoder := func(origLevel ekalog.Level) string {
		switch origLevel {
		case ekalog.LEVEL_DEBUG:
		case ekalog.LEVEL_INFO:
		case ekalog.LEVEL_WARNING:
		case ekalog.LEVEL_FATAL:
		}
	...
	if message != "" && e.ErrLetter != nil {
		for i, n := 0, len(e.ErrLetter.SystemFields); i < n; i++ {
			if e.ErrLetter.SystemFields[i].BaseType() == ekaunsafe.FIELD_KIND_SYS_TYPE_EKAERR_CLASS_NAME {
...
func (de *CI_DatadogEncoder) encodeStacktrace(
	...
	letterItem := (*ekaunsafe.LetterItem)(nil)
	if e.ErrLetter != nil {
...
func (de *CI_DatadogEncoder) encodeSysPrefixedFields(
	...
	s *jsoniter.Stream,
	fieldsMayContainSysPrefixed []ekaunsafe.Field,
...
func (_ *CI_DatadogEncoder) encodeNumWithZeroes(s *strings.Builder, num, maxNum int16) {
	...
	case 1:
		_, _ = s.WriteString("0")
	case 2:
		_, _ = s.WriteString("00")
	case 3:
		_, _ = s.WriteString("000")
	case 4:
		_, _ = s.WriteString("0000")
	}
